...
static void print_map() {
  int x, y;
  clear();
  mvprintw(0, 1, "Assignment 1.05 (The top line is for message display)");
  ...
  for (y = 0; y < MAP_Y; y++) {
    ...
    mvprintw(22, 1, "(The bottom 2 lines are for status information)");
    mvprintw(23, 1, "(deal with this in a later assignment)");
    refresh();
  }
}
...
void game_loop() {
  ...
  while (quit == 0) {
    ...
    if (c == &world.pc) {
      key = getch();
      switch(key){
        case 'y': // Attempt to move PC one cell to the upper left.
        case 'Y':
        case '7':
          new_y = c->pos[dim_y] - 1;
          new_x = c->pos[dim_x] - 1;

          if(move_cost[char_pc][world.cur_map->map[new_y][new_x]] != INT_MAX){
            if(world.cur_map->map[new_y][new_x] == ter_gate){
              ...
              break;
            }else if(world.cur_map->cmap[new_y][new_x] != NULL){
              battleInterface();
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = world.cur_map->cmap[new_y][new_x];
          ...
          break;
        case 'k': // Attempt to move PC one cell up.
        case 'K':
        case '8':
          new_y = c->pos[dim_y] - 1;
          new_x = c->pos[dim_x];
          ...
          break;
        case 'u': // Attempt to move PC one cell to the upper right.
        case 'U':
        case '9':
          new_y = c->pos[dim_y] - 1;
          new_x = c->pos[dim_x] + 1;
          ...
          break;
        case 'l': // Attempt to move PC one cell to the right.
        case 'L':
        case '6':
          new_y = c->pos[dim_y]; 
          new_x = c->pos[dim_x] + 1;
          ...
          break;
        case 'n': // Attempt to move PC one cell to the lower right.
        case 'N':
        case '3':
          new_y = c->pos[dim_y] + 1;
          new_x = c->pos[dim_x] + 1;
          ...
          if(move_cost[char_pc][world.cur_map->map[new_y][new_x]] != INT_MAX){
            if(world.cur_map->map[new_y][new_y] == ter_gate){
              world.cur_map->cmap[c->pos[dim_y]][c->pos[dim_x]] = NULL;
              if(new_y == 20){
                world.cur_idx[dim_y]++;               
                new_map();
          ...
          break;
        case 'j': // Attempt to move PC one cell down.
        case 'J':
        case '2':
          new_y = c->pos[dim_y] + 1;
          new_x = c->pos[dim_x];
          ...
          break;
        case 'b': // Attempt to move PC one cell to the lower left.
        case 'B':
        case '1':
          new_y = c->pos[dim_y] + 1;
          new_x = c->pos[dim_x] - 1;
          ...
          break;
        case 'h': // Attempt to move PC one cell to the left.
        case 'H':
        case '4':
          new_y = c->pos[dim_y];
          new_x = c->pos[dim_x] - 1;
          ...
          break;
        case '5': // Rest for a turn. NPCs still move. (PC doesn't move)
        case ' ':
          move = 1;
          break;
        case KEY_RIGHT: //Attempt to enter a Pokemart or Pokemon Center.
        ...
        case 'F':
          ...
          world.cur_idx[dim_y] = fly_y + 200;
          world.cur_idx[dim_x] = fly_x + 200;
          new_map();
          locatePath();
          moveMaps = 1;
          break;
        case 't': // Display a list of trainers on the map, with their symbol and position relative to the PC 
        case 'T':
          trainerList();
          break;
      ...
      if(move == 1){
        ...
        if(world.cur_map->cmap[dim_y][dim_x] != NULL){
              battleInterface();
        }
      }
    }else{
...
void centerInterface(){
  ...
  int exit = 0;
  int input;
  mvprintw(0, 1, "PokeCenter interface: functionality will be added in later assignments, use the left arrow key to exit this interface.");
  refresh();
  while(exit == 0){
    input = getch();
    if(input == KEY_LEFT){
...
void martInterface(){
  ...
  int exit = 0;
  int input;
  mvprintw(0, 1, "PokeMart interface: functionality will be added in later assignments, use the left arrow key to exit this interface.");
  refresh();
  while(exit == 0){
    input = getch();
    if(input == KEY_LEFT){
...
void battleInterface(){
  ...
  int exit = 0;
  int input;
  mvprintw(0, 1, "Battle interface: functionality will be added in later assignments, use the left arrow key to exit this interface.");
  refresh();
  while(exit == 0){
    input = getch();
    if(input == KEY_LEFT){
...
void errorInterface(){
  ...
  int exit = 0;
  int input;
  mvprintw(0, 1, "An error has occurred due to invalid inputs, use the left arrow key to exit this interface.");
  refresh();
  while(exit == 0){
    input = getch();
    if(input == KEY_LEFT){
...
void trainerList(){
  ...
  int exit = 0;
  int start_row = 0;
  int max_rows = 23; // terminal height 
  ...
  while (!exit) {
    ...
    for (i = start_row; i < MAP_Y; ++i) {
      for (j = 0; j < MAP_X; ++j) {
        if (world.cur_map->cmap[i][j] != NULL) {
          y = i - world.pc.pos[dim_y];
          x = j - world.pc.pos[dim_x];
          if (y < 0) {
            north_south = "North";
            y *= (-1);
          } else {
          ...
          mvprintw(g, 1, "%c: %d %s and %d %s", world.cur_map->cmap[i][j]->symbol, y, north_south, x, east_west);
          g++;
        }
      }
    }
    ...
    input = getch();
    switch (input) {
      case KEY_UP: // When displaying trainer list, if entire list does not fit in screen and not currently at top of list, scroll list up.
        if (start_row > 0) {
          start_row--;
        }
        break;
      case KEY_DOWN: // When displaying trainer list, if entire list does not fit in screen and not currently at bottom of list, scroll list down.
        if (start_row < MAP_Y - max_rows) {
          start_row++;
        }
        break;
      case 27: // When displaying trainer list, return to character control. (27 references ESC key)
        exit = 1;
        break;
    }
    }
