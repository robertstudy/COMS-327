...
int main(int argc, char *argv[])
{
  ...
  char file[50];
  printf("Which file would you like to parse? Options are listed: \n");
  printf("pokemon, moves, pokemon_moves, pokemon_species, experience, pokemon_stats, stats, pokemon_types \n");
  scanf("%s", file);

  if(strcmp(file, "pokemon") == 0) {
    parse_pokemon();
  } else if(strcmp(file, "moves") == 0) {
    parse_moves();
  } else if(strcmp(file, "pokemon_moves") == 0) {
    parse_pokemon_moves();
  } else if(strcmp(file, "pokemon_species") == 0) {
    parse_pokemon_species();
  } else if(strcmp(file, "experience") == 0) {
    parse_experience();
  } else if(strcmp(file, "type_names") == 0) {
    parse_type_names();
  } else if(strcmp(file, "pokemon_stats") == 0) {
    parse_pokemon_stats();
  } else if(strcmp(file, "stats") == 0) {
    parse_stats();
  } else if(strcmp(file, "pokemon_types") == 0) {
    parse_pokemon_types();
  } else {
    printf("Invalid file input: %s, Escape with ^c\n", file);
  }
